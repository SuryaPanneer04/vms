...
if ($pass_no) {
    $count_query .= " AND pass_no LIKE ?";
    $params[] = "%$pass_no%";
}
if ($contact_no) {
    $count_query .= " AND contact_no LIKE ?";
    $params[] = "%$contact_no%";
}
if ($from_date) {
    $count_query .= " AND (DATE(in_time) >= ? OR DATE(meeting_date_time) >= ?)";
    $params[] = $from_date;
    $params[] = $from_date;
}
if ($to_date) {
...
$final_params = [];
if ($pass_no) {
    $query .= " AND v.pass_no LIKE ?";
    $final_params[] = "%$pass_no%";
}
if ($contact_no) {
    $query .= " AND v.contact_no LIKE ?";
    $final_params[] = "%$contact_no%";
}
if ($from_date) {
    $query .= " AND (DATE(v.in_time) >= ? OR DATE(v.meeting_date_time) >= ?)";
    $final_params[] = $from_date;
    $final_params[] = $from_date;
}
if ($to_date) {
    $query .= " AND (DATE(v.in_time) <= ? OR DATE(v.meeting_date_time) <= ?)";
    $final_params[] = $to_date;
    $final_params[] = $to_date;
}
                    ...
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            <?php foreach ($visitors as $row): ?>
                                <tr>
                                    <td>
                                        <span class="fw-bold text-primary">#<?= htmlspecialchars($row['pass_no'] ?: "null") ?></span>
            ...
            fetch("get_meeting_timeline.php?visitor_id=" + id)
                ...
                .then(data => {
                    ...
                    if (data.length === 0) {
                        ...
                        data.forEach(item => {
                            let out = item.check_out_time ? new Date(item.check_out_time).toLocaleTimeString([], {
                                hour: '2-digit',
                                minute: '2-digit'
